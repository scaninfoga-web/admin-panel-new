            ...
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-foreground text-xs">
                  {formatDate(statement.request_at)}
                </p>
              </div>
            </div>
          )}
          {statement.approved_at && (
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-foreground text-xs">
                  {formatDate(statement.approved_at)}
                </p>
              </div>
            </div>
          )}
          {statement.rejected_at && (
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-foreground text-xs">
                  {formatDate(statement.rejected_at)}
                </p>
              </div>
            </div>
          )}
          {statement.success_at && (
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-foreground text-xs">
                  {formatDate(statement.success_at)}
                </p>
              </div>
            </div>
          )}
...
export default function BankPdfS3() {
  ...
  const [deleteUploadLoading, setDeleteUploadLoading] =
    useState<boolean>(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // New Record state
  const [newRecordUploadDialogOpen, setNewRecordUploadDialogOpen] =
    useState<boolean>(false);
  const [newRecordFormDialogOpen, setNewRecordFormDialogOpen] =
    useState<boolean>(false);
  ...
  const [newRecordUploadedFile, setNewRecordUploadedFile] = useState<
    UploadResponse["responseData"] | null
  >(null);
  const [newRecordSubmitLoading, setNewRecordSubmitLoading] =
    useState<boolean>(false);
  const [newRecordDeleteLoading, setNewRecordDeleteLoading] =
    useState<boolean>(false);
  ...
  const handleProceed = async () => {
    ...
    try {
      const res = await post<StatementsResponse>(
        "/api/mobile/get-pdf-bank-statement",
        {
    ...
    try {
      ...
      switch (confirmAction) {
        ...
        case "delete":
          ...
          payload = {
            ...payload,
            s3_key: selectedStatement.s3_key,
            request_type: "delete",
          };
          break;
      }
  ...
  const handleSubmitUpload = async () => {
    ...
    try {
      const res = await post("/api/mobile/set-pdf-bank-statement", {
        ...
        investigator_officier_name:
          selectedStatement.investigator_officier_name,
        bank_name: selectedStatement.bank_name,
        ifsc_code: selectedStatement.ifsc_code,
      });

      toast.success(
        res?.responseStatus?.message || "Statement uploaded successfully"
      );
      resetUploadDialog();
      refreshStatements();
    } catch (error: any) {
      const errMsg =
        error?.response?.data?.responseStatus?.message || "Submission failed";
      toast.error(errMsg);
    } finally {
  ...
  const handleDeleteUploadedFile = async () => {
    ...
    try {
      ...
      toast.success(
        res?.responseStatus?.message || "File deleted successfully"
      );
      ...
      const errMsg =
        error?.response?.data?.responseStatus?.message || "Delete failed";
      toast.error(errMsg);
    } finally {
  ...
  // New Record handlers
  const handleNewRecordFileChange = (
  ...
  const handleNewRecordSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        res?.responseStatus?.message || "New record created successfully"
      );
      resetNewRecordState();
      refreshStatements();
    } catch (error: any) {
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] flex flex-col">
            ...
            {fetchingStatements ? (
              ...
              >
                ...
                <p className="text-xs mt-1">
                  No bank statements available for this number
                </p>
              </motion.div>
            )}
          </div>
          ...
          <DialogFooter className="border-t pt-4">
            ...
            <Button onClick={refreshStatements} disabled={fetchingStatements}>
              <RotateCcw
                className={cn("h-4 w-4", fetchingStatements && "animate-spin")}
              />
              Refresh
            </Button>
          </DialogFooter>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && resetUploadDialog()}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
      ...
      <Dialog
        open={newRecordUploadDialogOpen}
        onOpenChange={(open) => !open && resetNewRecordState()}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={resetNewRecordState}
              disabled={newRecordUploading}
            >
              Cancel
            </Button>
          </DialogFooter>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="newRecordInvestigatorName">
